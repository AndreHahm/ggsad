...
- **Global scope**: `C:/Dev/Repos/ggsad/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Dev/Repos/ggsad/.claude/notes/`)
...
2. Glob `C:/Dev/Repos/ggsad/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Dev/Repos/ggsad/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Dev/Repos/ggsad/.claude/notes/` — works in any directory
