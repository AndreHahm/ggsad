...
- **Quality/verification baseline commands:** `uv sync --locked`, `uv run ruff format --check .`, `uv run ruff check .`, `uv run ty check`, `uv run pytest`, `uv build` (per `AGENTS.md` and the transition SPEC; supersedes the bare `uv sync` and `mypy` framing still present in some DOC-classified legacy sources).
